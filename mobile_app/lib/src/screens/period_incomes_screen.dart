...
import '../cubits/period_incomes/period_incomes_cubit.dart';
...
class PeriodIncomesScreen extends StatelessWidget {
  const PeriodIncomesScreen({required this.currency, super.key});
  ...
  Widget build(BuildContext context) {
    ...
    return BlocProvider(
      create: (_) => PeriodIncomesCubit(repository: repo),
      child: Builder(
        builder: (context) {
          return Scaffold(
            appBar: AppBar(
              title: const Text('Incomes by period'),
              actions: [
                IconButton(
                  icon: const Icon(Icons.refresh),
                  tooltip: 'Refresh',
                  onPressed: () => context.read<PeriodIncomesCubit>().refresh(),
                ),
              ],
            ),
            body: ResponsiveContentWidth(
              child: BlocBuilder<PeriodIncomesCubit, PeriodExpensesState>(
                buildWhen: (p, n) =>
                ...
                builder: (context, state) {
                  final cubit = context.read<PeriodIncomesCubit>();
                              ...
                              return Column(
                                ...
                                children: [
                                  ...
                                  if (incomes.isEmpty)
                                    const _EmptyCard(
                                      message:
                                          'No incomes in this range in local data.',
                                    )
                                  else
                                    ...incomes.map(
                                      (income) => _IncomeTile(
...
class _IncomeTile extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final color = _parseColor(income.categoryColor);
    ...
    return Card(
      ...
      child: ListTile(
        ...
        subtitle: Text('${income.categoryName} • ${income.date}'),
        trailing: Text(
          formatMoney(currency, income.amount),
